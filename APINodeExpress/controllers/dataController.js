...
const updateUser = async(req, res) =>{
  ...
  try{
    ...
    const userExist = await pool.query("SELECT * FROM cliente WHERE email = $1", [userEmail]);

    if(userExist.rowCount === 0){

      return res.status(404).json({ error: 'No se encontro al usuario en la base de datos'});
    }

    //Realizar actualizacion datos del usuario
    await pool.query("UPDATE cliente SET username = $1, email = $2 WHERE email = $3", [username, email, userEmail]);

    res.status(200).json({ message: 'Informacion de usuario actualizado con exito'});

  } catch(errro){

    console.error('Error al actualizar datos del cliente ', errro);
...
const getHistoryUser = async(req, res) =>{
  ...
  try{
    ...
    const historial = await pool.query("SELECT p.nombre, i.ruta, v.precio, v.cantidad, v.total, v.fecha_venta FROM producto p JOIN imagenes i ON p.id = i.id JOIN ventas v ON p.id = v.id_producto WHERE v.id_cliente = $1;", [userId])
    ...
    if(historial.rows.length > 0){
      ...
      return res.status(404).json({ error: 'No se pudo encontrar el historial del usuario con el id: ',userId})
    }
